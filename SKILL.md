...
1. **AI 自动扫描与匹配：**
   ...
   >
2. **向用户确认：** 将扫描到的技能通过 `AskUserQuestion` 展示给用户，询问是否添加。列出的技能来自**当前工作目录**下 `.opencode/skills/dev/` 的实际子目录，**不是预定义的技能名**。示例（假设目录下有 若干 技能子目录）：
...
- `scripts/init-agent.js` — 初始化搭档
- `scripts/add-skill.js` — 添加技能（本地 / 远程）
- `scripts/list-agents.js` — 查看搭档清单
