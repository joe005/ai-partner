...
- 每次调用最多包含 4 个问题，优先合并可并行询问的内容
- 每个选项必须包含 `label`（1-5 个词）与 `description`（说明该选项含义或影响）
- 推荐选项放在首位并在 label 末尾标注「(推荐)」
- 互斥选项使用单选；可同时启用的选项（如多个候选技能）使用 `multiSelect: true`
- 系统自动提供「Other」，无需手动添加；用户始终可自由补充
- 已由 AI 自动推断且置信度高的字段（如显示名称、标识符、头像、领域），禁止询问

**询问黑名单（强制，需求收集阶段绝对不得问）：**

- 头像：一律随机分配预置头像，不询问用户偏好、不询问是否提供自定义图片
- 领域：AI 根据功能描述静默归类到一个领域标签，不询问用户确认、不列候选领域让用户选
- 知识库：不询问是否上传、不询问上传哪些文档；用户若在消息中主动提供才处理，否则跳过

**典型询问场景：**

1. 功能描述不清晰时，提供 2-4 个典型方向供用户确认
2. 匹配技能候选列表的确认（见下文「技能推荐」步骤，使用 `multiSelect: true`）

**基础信息（必须）：**

- 功能描述（用户提供，不清晰时可确认方向）
- 领域标签：AI 根据角色设定和功能描述静默匹配以下选项之一 — 财务 / 人力资源 / 销售 / 供应链 / 行政 / 法务 / IT / 通用（禁止问用户）
...
- 头像：一律从内置 `assets/avatars/` 随机分配一张，复制为 `avatar.png`；用户若主动提供本地图片路径才走自定义分支。
- 知识库：初始化时默认为空；用户若在消息中主动提供文档才处理，否则不问、不提、不提示。

**技能推荐（AI 自动推断 + 用户确认）：**

AI 根据用户的功能描述自动推断该智能体可能需要的技能，扫描当前工作目录下 `skills/` 的**所有子目录**（每个子目录即一个技能），读取每个技能的 `SKILL.md` 中的 `name` 与 `description`，按用户需求自动匹配。
...
1. **AI 自动扫描与匹配：**
   ...
   1. 列出**当前工作目录**（即用户执行命令时所在的项目根目录）下 `skills/` 的所有子目录（每个子目录即一个技能）
   2. 读取每个子目录下的 `SKILL.md`，提取 YAML frontmatter 中的 `name` 与 `description`
   3. 按用户需求语义匹配 `name` / `description`，筛选候选技能
   4. 如该目录不存在或为空，直接跳过本步骤

   > 技能列表完全来自该目录下的实际内容，始终以**当前工作目录**为准。
   >
2. **向用户确认：** 将扫描到的技能通过 `ask_user_question` 工具（`multiSelect: true`）展示给用户，询问是否添加。列出的技能来自**当前工作目录**下 `skills/` 的实际子目录，**不是预定义的技能名**。每个候选技能作为一个选项，`label` 为技能名、`description` 为 SKILL.md 中的 `description`。示例（假设目录下有 若干 技能子目录）：

   > 根据用户描述，从**当前工作目录**下 `skills/` 匹配到以下技能，请选择要添加的：
   >
...
如**当前工作目录**下 `skills/` 目录为空、无可匹配技能，或用户不需要添加，直接跳过此步骤，无需询问。
...
> **参数由 AI 自动生成：** 根据步骤 1 收集的信息，AI 自动生成标识符、显示名称、领域标签和功能描述，用户无需手动填写。
...
所有对 `assistant.json` 的修改（包括本步骤、第 4 步技能追加、第 5 步知识库追加，以及后续任何维护操作），必须严格遵循以下流程：

1. 完整读取文件内容
2. 用 `JSON.parse` 解析为对象；解析失败则立即停止并修复
3. 在对象上做字段修改
4. 用 `JSON.stringify(obj, null, 2)` 序列化
5. **完整覆盖写入**整个文件（末尾保留一个换行）

**禁止**对 `assistant.json` 做基于行匹配、字符串拼接、局部片段插入的增量编辑（含但不限于 `search_replace`、行级 `edit`、手写逗号/括号）。JSON 语法对逗号、引号、括号极度敏感，增量编辑一次漏配即产出坏产物。
...
> **本地技能目录：** **当前工作目录**下的 `skills/`（即用户执行命令时所在的项目根目录下的该子路径），AI 直接扫描匹配。
...
```bash
# 从本地技能目录添加（<skill-name> 为**当前工作目录**下 skills/ 的实际技能目录名）
node scripts/add-skill.js <agent-name> --source ./skills/<skill-name>
```

**自动匹配（初始化时）：**

```bash
# AI 扫描**当前工作目录**下的 skills/ 中的所有技能并按需求自动匹配。
...
2. **必填字段齐全**：`type` / `id` / `name` / `displayName` / `avatar` / `description` / `domain` / `visibility` / `role` / `skills` / `knowledge` / `version` / `createdAt` / `updatedAt` 均存在（其中 `skills` 与 `knowledge` 允许为空数组 `[]`，其余字段非空）；`name` 为 kebab-case 且与目录名严格一致；`id === name` 恒等约束成立；`type` 固定为字符串 `"assistant"`；`avatar` 必须指向智能体目录内真实存在的头像文件
...
- 输出 `:::render:ai-partner` 之前，**必须**重新读取 `assistant.json` 并对其内容执行 `JSON.parse` 校验
- 校验失败（语法错误、编码异常、截断等）则**禁止渲染**，必须先修复并重跑校验通过后才能输出渲染指令
- 同步校验 `agent.md` 存在、`avatar` 指向的头像文件真实存在、`skills[]` 与 `knowledge[]` 引用的磁盘文件均可读
- 任一项不通过，修复后重验；不得跳过、不得以「小问题」为由放行
...
- **发现技能：** AI 直接读取**当前工作目录**下 `skills/` 中每个子目录的 `SKILL.md`，按 `name` / `description` 语义匹配
...
```bash
# 1. AI 扫描本地技能目录：
#    读取**当前工作目录**下 skills/ 中每个子目录的 SKILL.md，按需求匹配候选技能
#    （注意：当前工作目录 = 用户执行命令时所在的项目根）
...
- `./skills/` — **当前工作目录**下的本地技能根目录（AI 自动扫描匹配）

## 更新日志

| 版本 | 日期       | 类型  | 变更内容                                                       |
| :---: | ---------- | ----- | -------------------------------------------------------------- |
| 1.0.4 | 2026-05-06 | PATCH | 本地技能目录迁移为当前工作目录下的 `skills/`                 |
| 1.0.3 | 2026-05-06 | PATCH | 需求收集阶段新增询问黑名单，禁止询问头像/领域/知识库           |
| 1.0.2 | 2026-05-06 | PATCH | 新增写入方式与渲染闸门两条硬规则，保障 assistant.json 语法合法 |
| 1.0.1 | 2026-05-06 | PATCH | 大纲优化：统一标题层级与措辞风格                               |
